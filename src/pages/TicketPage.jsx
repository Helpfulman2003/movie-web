...
function TicketPage() {
 ...
 const filteredTickets = tickets?.filter((ticket) => {
  const matchesSearch = searchTerm
    ? ticket.maSuatChieu?.maPhim?.tenPhim?.toLowerCase().includes(searchTerm.toLowerCase())
    : true; // Nếu không có từ khóa tìm kiếm, trả về true 
    const matchesDate = filterDate
    ? ticket.maSuatChieu?.gioBatDau && new Date(ticket.maSuatChieu?.gioBatDau).toISOString().split('T')[0] === filterDate // Chuyển showTime sang định dạng YYYY-MM-DD
  ...
  return (
    <div className='min-h-screen bg-gradient-to-r from-blue-100 to-blue-200 p-6'>
      ...
      <div className='bg-white p-6 rounded-lg shadow-md mb-8'>
        ...
        {status === 'succeeded' && (
          <TicketList tickets={filteredTickets} deleteTicket={deleteTicket} collectTicket={handleCollectTicket}/>
        )}
      </div>
      {/* Nút thêm vé */}
